...
@interface JiveInvalidAFImageRequestOperation : AFImageRequestOperation<JiveRetryingOperation>

@property (copy, nonatomic) JiveCompletedBlock errorReporter;
...
@implementation JiveInvalidAFImageRequestOperation

@synthesize retrier, retryCallbackQueue;

- (void)start {
    dispatch_after(DISPATCH_TIME_NOW, dispatch_get_main_queue(), self.errorReporter);
}
...
+ (id)createOperationForInvalidRequest:(JiveCompletedBlock)errorReporter {
    JiveInvalidAFImageRequestOperation *operation = [JiveInvalidAFImageRequestOperation new];
    
    operation.errorReporter = errorReporter;
    return operation;
}
