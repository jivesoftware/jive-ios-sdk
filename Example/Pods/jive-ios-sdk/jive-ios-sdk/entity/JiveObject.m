...
#if JIVE_JSON_DEBUG
        NSLog(@"Extra field in %@ - %@", [self class], key);
#endif
...
- (void) setValue:(id)value forUndefinedKey:(NSString *)key {
    ...
    if([key isEqualToString:JiveObjectConstants.id]) {
        [self setValue:value forKey:JiveObjectAttributes.jiveId];
    } else if ([key isEqualToString:JiveObjectConstants.description]) {
        [self setValue:value forKey:JiveObjectAttributes.jiveDescription];
    }
}
...
- (Class) lookupPropertyClass:(NSString*) propertyName {
    ...
    if (!ivar) {
        if ([propertyName isEqualToString:JiveObjectConstants.id]) {
            ivar = class_getInstanceVariable([self class], JiveObjectAttributes.jiveId.UTF8String);
        } else if ([propertyName isEqualToString:JiveObjectConstants.description]) {
            ivar = class_getInstanceVariable([self class], JiveObjectAttributes.jiveDescription.UTF8String);
        }
    }
