...
@synthesize addresses, displayName, emails, followerCount, followingCount, jiveId, jive, location;
@synthesize name, phoneNumbers, photos, published, status, tags, thumbnailUrl, updated, thumbnailId;
...
- (Class) arrayMappingFor:(NSString*) propertyName {
    ...
    Class classForProperty = propertyClasses[propertyName];
    
    return classForProperty ? classForProperty : [super arrayMappingFor:propertyName];
}
...
- (NSDictionary *)toJSONDictionary {
    ...
    [dictionary setValue:self.jiveId forKey:JiveObjectConstants.id];
...
- (id)persistentJSON {
    ...
    [dictionary setValue:followerCount forKey:JivePersonAttributes.followerCount];
    [dictionary setValue:followingCount forKey:JivePersonAttributes.followingCount];
    [dictionary setValue:[dateFormatter stringFromDate:published]
                  forKey:JivePersonAttributes.published];
    [dictionary setValue:thumbnailId forKey:JivePersonAttributes.thumbnailId];
    [dictionary setValue:thumbnailUrl forKey:JivePersonAttributes.thumbnailUrl];
    [dictionary setValue:[dateFormatter stringFromDate:updated]
                  forKey:JivePersonAttributes.updated];
    if (jive)
        [dictionary setValue:[jive persistentJSON] forKey:JivePersonAttributes.jive];
    
    if (name)
        [dictionary setValue:[name persistentJSON] forKey:JivePersonAttributes.name];
...
- (void) avatarOnComplete:(JiveNativeImageCompleteBlock)completeBlock
                  onError:(JiveErrorBlock)errorBlock {
...
- (AFImageRequestOperation<JiveRetryingOperation> *) avatarOperationOnComplete:(JiveNativeImageCompleteBlock)completeBlock
                                                                       onError:(JiveErrorBlock)errorBlock {
