...
- (void)testCount {
    ...
    GHAssertEqualObjects(@"startIndex=5", asString, @"Wrong string contents");
    
    value = 7;
    self.pagedOptions.startIndex = value;
    asString = [self.options toQueryString];
    GHAssertNotNil(asString, @"Invalid string returned");
    GHAssertEqualObjects(@"startIndex=7", asString, @"Wrong string contents");
}
...
- (void)testCountWithField {
    ...
    GHAssertNotNil(asString, @"Invalid string returned");
    GHAssertEqualObjects(@"fields=name&startIndex=5", asString, @"Wrong string contents");
}
