...
- (void) testOCMockExample {
    ...
    GHAssertEquals(expectedValue, [mockString length], @"Mock string length incorrect.");
...
- (void) testJAPIRequestOperation {
    ...
    [mock setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        GHAssertNotNil(responseObject, @"JAPIRequestOperation returned nil response.");
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        GHFail(@"Unable to load test data. %@", [error localizedDescription]);
    }];
...
- (void) testJiveCredentialsBasicAuth {
    ...
    GHAssertTrue([[request allHTTPHeaderFields] count] == 0, @"NSMutableURLRequest should have 0 header count");
    
    [credentials applyToRequest:request];
    
    
    GHAssertTrue([[request allHTTPHeaderFields] count] == 1, @"NSMutableURLRequest should have at least 1 header");
    
    NSString *header = [[request allHTTPHeaderFields] objectForKey:@"Authorization"];
    
    GHAssertNotNil(header, @"Authorization should not be nil");
    
    GHAssertTrue([header isEqualToString:@"Basic cm9iOmJsYWhibGFoOTg3NjU0"],
