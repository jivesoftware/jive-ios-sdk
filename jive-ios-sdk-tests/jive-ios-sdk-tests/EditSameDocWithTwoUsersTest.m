...
- (void) testEditSameDocWithTwoUsers{
    ...
    //get userid2 person info
    __block JivePerson *personUser2 = nil;
    [self waitForTimeout:^(void (^finishedBlock)(void)) {
        [jive2 me:^(JivePerson *person) {
            personUser2 = person;
            finishedBlock();
        } onError:^(NSError *error) {
            STFail([error localizedDescription]);
    ...
    //add jive2 as the co-author
    newlyCreatedDoc.authors = [newlyCreatedDoc.authors arrayByAddingObject:personUser2];
    ...
    //expected: "status":409 .  01/09/2014: error with 403 (Forbidden)
